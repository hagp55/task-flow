...
def test_get_google_redirect_utr__success(
    ...
    auth_service_google_redirect_url: str = auth_service.get_google_redirect_url()
...
def test_get_yandex_redirect_utr__success(
    ...
    auth_service_yandex_redirect_url: str = auth_service.get_yandex_redirect_url()
...
def test_generate_access_token__success(
    ...
    user_id = 1
    access_token: str = auth_service.generate_access_token(user_id)
...
def test_get_user_if_from_access_token__success(
    ...
    user_id = 1
    access_token: str = auth_service.generate_access_token(user_id)
    decoded_user_id: int = auth_service.get_user_id_from_access_token(access_token)

    assert decoded_user_id == user_id


async def test_google_auth__success(auth_service: AuthService, user: User) -> None:
    user_data: UserLoginOut = await auth_service.google_auth(code="fake_code")
